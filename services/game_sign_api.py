...
class BaseGameSign:
    ...
    def __init__(self, account: UserAccount, records: List[GameRecord]):
        ...
        reward_params = {"lang": "zh-cn", "act_id": self.act_id}
    ...
    ) -> Tuple[BaseApiStatus, Optional[List[Award]]]:
        """
        获取签到奖励信息
    ...
    async def get_info(
        self, platform: Literal["ios", "android"] = "ios", retry: bool = True
        ...
        headers["x-rpc-device_id"] = (
            self.account.device_id_ios
            if platform == "ios"
            else self.account.device_id_android
        )
            ...
            async for attempt in get_async_retry(retry):
                with attempt:
                    headers["DS"] = (
                        generate_ds()
                        if platform == "ios"
                        else generate_ds(platform="android")
                    )
                    async with httpx.AsyncClient() as client:
                        res = await client.get(
                            self.url_info,
                    ...
                    return BaseApiStatus(success=True), GameSignInfo.parse_obj(
                        api_result.data
                    )
        except tenacity.RetryError as e:
            if is_incorrect_return(e):
            ...
            async for attempt in get_async_retry(retry):
                with attempt:
                    ...
                    if api_result.login_expired:
                        ...
                        logger.debug(f"网络请求返回: {res.text}")
                        return BaseApiStatus(invalid_ds=True), None
                    elif api_result.data and api_result.data.get("risk_code") != 0:
                        logger.warning(
                            f"游戏签到 - 用户 {self.account.display_name} 可能被人机验证阻拦"
                        )
                        logger.debug(f"网络请求返回: {res.text}")
                        return BaseApiStatus(need_verify=True), MmtData.parse_obj(
                            api_result.data
                        )
                    else:
                        logger.success(
