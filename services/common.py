...
URL_MULTI_TOKEN_BY_LOGIN_TICKET = (
    "https://api-takumi.mihoyo.com/auth/api/getMultiTokenByLoginTicket?login_ticket={0}"
    "&token_types=3&uid={1}"
)
URL_COOKIE_TOKEN_BY_CAPTCHA = (
    "https://api-takumi.mihoyo.com/account/auth/api/webLoginByMobile"
)
...
URL_ACTION_TICKET = (
    "https://api-takumi.mihoyo.com/auth/api/getActionTicketBySToken?action_type=game_role"
...
URL_GOOD_LIST = (
    "https://api-takumi.mihoyo.com/mall/v1/web/goods/list?app_id=1&point_sn=myb&page_size=20&page={"
    "page}&game={game} "
)
...
URL_REGISTRABLE = (
    "https://webapi.account.mihoyo.com/Api/is_mobile_registrable?mobile={mobile}&t={t}"
)
URL_CREATE_MMT = (
    "https://webapi.account.mihoyo.com/Api/create_mmt?scene_type=1&now={now}"
...
class ApiResultHandler(BaseModel):
    ...
    @classmethod
    def from_response(cls, response_data: Dict[str, Any]) -> "ApiResultHandler":
        """从API响应创建处理器"""
        # 记录完整的服务器返回数据（debug级别）
        logger.debug(f"服务器返回原始数据: {response_data}")
        ...
        for key in ["retcode", "status"]:
            if retcode is None:
                retcode = response_data.get(key)
                if retcode is None and data:
                    retcode = data.get(key)

        # 解析 message
        message = None
        for key in ["message", "msg"]:
            if message is None:
                message = response_data.get(key)
                if message is None and data:
                    message = data.get(key)

        return cls(content=response_data, data=data, retcode=retcode, message=message)
    ...
    def wrong_captcha(self):
        ...
        是否返回验证码错误
        """
        return self.retcode in [-201, -302] or self.message in [
            "验证码错误",
            "Captcha not match Err",
        ]
...
async def get_game_record(
    account: UserAccount, retry: bool = True
) -> Tuple[BaseApiStatus, Optional[List[GameRecord]]]:
    """
    ...
    try:
        ...
        if is_incorrect_return(e):
            ...
            if res is not None:  # 检查 res 是否存在
                logger.debug(f"网络请求返回: {res.text}")
            else:
                logger.debug("网络请求未完成，无法获取返回内容")
...
async def get_game_list(
    retry: bool = True,
) -> Tuple[BaseApiStatus, Optional[List[GameInfo]]]:
    """
...
async def get_user_myb(
    account: UserAccount, retry: bool = True
) -> Tuple[BaseApiStatus, Optional[int]]:
    """
    获取用户当前米游币数量
...
async def check_registrable(
    phone_number: int, keep_client: bool = False, retry: bool = True
) -> Tuple[BaseApiStatus, Optional[bool], str, Optional[httpx.AsyncClient]]:
    """
    检查用户是否可以注册
    ...
    try:
        async for attempt in get_async_retry(retry):
            with attempt:
                ...
                api_result = ApiResultHandler.from_response(res.json())
                return (
                    BaseApiStatus(success=True),
                    bool(api_result.data["is_registable"]),
                    device_id,
                    client,
                )
    except tenacity.RetryError as e:
        ...
        if is_incorrect_return(e):
            logger.exception(
                f"检查用户 {phone_number} 是否可以注册 - 服务器没有正确返回"
            )
            logger.debug(f"网络请求返回: {res.text}")
...
async def create_mmt(
    client: Optional[httpx.AsyncClient] = None,
    use_v4: bool = True,
    device_id: str = None,
    retry: bool = True,
) -> Tuple[BaseApiStatus, Optional[MmtData], str, Optional[httpx.AsyncClient]]:
    """
    发送短信验证前所需的人机验证任务申请
    ...
    try:
        async for attempt in get_async_retry(retry):
            with attempt:
                ...
                return (
                    BaseApiStatus(success=True),
                    MmtData.parse_obj(api_result.data["mmt_data"]),
                    device_id,
                    client,
                )
    except tenacity.RetryError as e:
        ...
        if is_incorrect_return(e):
            logger.exception(
                "获取短信验证-人机验证任务(create_mmt) - 服务器没有正确返回"
            )
            logger.debug(f"网络请求返回: {res.text}")
            return BaseApiStatus(incorrect_return=True), None, device_id, client
        else:
...
async def create_mobile_captcha(
    phone_number: str,
    mmt_data: MmtData,
...
async def get_login_ticket_by_captcha(
    phone_number: str,
    captcha: int,
    device_id: str = None,
    client: Optional[httpx.AsyncClient] = None,
...
async def get_multi_token_by_login_ticket(
    cookies: BBSCookies, retry: bool = True
) -> Tuple[GetCookieStatus, Optional[BBSCookies]]:
    """
    ...
    try:
        async for attempt in get_async_retry(retry):
            with attempt:
                async with httpx.AsyncClient() as client:
                    res = await client.get(
                        URL_MULTI_TOKEN_BY_LOGIN_TICKET.format(
                ...
                if api_result.login_expired:
                    ...
                    cookies.stoken = list(
                        filter(lambda x: x["name"] == "stoken", api_result.data["list"])
                    )[0]["token"]
                    cookies.ltoken = list(
                        filter(lambda x: x["name"] == "ltoken", api_result.data["list"])
                    )[0]["token"]
                    return GetCookieStatus(success=True), cookies
    except tenacity.RetryError as e:
        if is_incorrect_return(e):
...
async def get_cookie_token_by_captcha(
    phone_number: str, captcha: int, retry: bool = True
) -> Tuple[GetCookieStatus, Optional[BBSCookies]]:
    """
...
async def get_login_ticket_by_password(
    account: str,
    password: str,
    mmt_data: MmtData,
...
async def get_device_fp(
    device_id: str, retry: bool = True
) -> Tuple[GetFpStatus, Optional[str]]:
    """
    获取 x-rpc-device_fp
    ...
    try:
        async for attempt in get_async_retry(retry):
            with attempt:
                ...
                api_result = ApiResultHandler.from_response(res.json())
                if (
                    api_result.data["code"] == 403
                    or api_result.data["msg"] == "传入的参数有误"
                ):
                    ...
                    if not device_fp:
                        logger.error(
                            "获取 x-rpc-device_fp: 服务器返回的 device_fp 为空"
                        )
...
async def genshin_note(
    account: UserAccount,
) -> Tuple[Union[BaseApiStatus, GenshinNoteStatus], Optional[GenshinNote]]:
    """
    获取原神实时便笺
        ...
        if record.game_id == game_id:
            try:
                ...
                headers["x-rpc-device_fp"] = (
                    account.device_id_android or generate_fp_locally()
                )
                async for attempt in get_async_retry(False):
                    with attempt:
                        headers["DS"] = generate_ds(
                            params={
                                "role_id": record.game_role_id,
                        ...
                        return GenshinNoteStatus(success=True), GenshinNote.parse_obj(
                            api_result.data
                        )
            except tenacity.RetryError as e:
                if is_incorrect_return(e):
...
async def starrail_note(
    account: UserAccount,
) -> Tuple[Union[BaseApiStatus, StarRailNoteStatus], Optional[StarRailNote]]:
    """
    获取崩铁实时便笺
                ...
                async for attempt in get_async_retry(False):
                    with attempt:
                        ...
                        if api_result.retcode == 1034:
                            ...
                            logger.debug(f"网络请求返回: {res.text}")
                        return StarRailNoteStatus(success=True), StarRailNote.parse_obj(
                            api_result.data
                        )
            except tenacity.RetryError as e:
                if is_incorrect_return(e):
...
async def create_verification(
    account: UserAccount = None, retry: bool = True
    ...
    try:
        async for attempt in get_async_retry(retry):
            with attempt:
                ...
                headers["x-rpc-device_fp"] = (
                    account.device_fp
                    if account and account.device_fp
                    else generate_fp_locally()
                )
                headers["DS"] = generate_ds()
                async with httpx.AsyncClient() as client:
                    res = await client.get(
        ...
        if is_incorrect_return(e):
            logger.exception(
                "创建人机验证任务(create_verification) - 服务器没有正确返回"
            )
            logger.debug(f"网络请求返回: {res.text}")
            return BaseApiStatus(incorrect_return=True), None
        else:
...
async def verify_verification(
    ...
    try:
        ...
        if is_incorrect_return(e):
            logger.exception(
                "验证人机验证结果(verify_verification) - 服务器没有正确返回"
            )
            logger.debug(f"网络请求返回: {res.text}")
            return BaseApiStatus(incorrect_return=True)
        else:
...
async def fetch_game_token_qrcode(
    device_id: str, app_id: str, retry: bool = True
    ...
    try:
        ...
        if is_incorrect_return(e):
            logger.exception(
                "获取米游社扫码登录(fetch_game_token_qrcode) - 服务器没有正确返回"
            )
            logger.debug(f"网络请求返回: {res.text}")
            return BaseApiStatus(incorrect_return=True), None
        else:
...
async def query_game_token_qrcode(
    ticket: str, device_id: str, app_id: str = "1", retry: bool = True
    ...
    try:
        async for attempt in get_async_retry(retry):
            with attempt:
                content = {"app_id": app_id, "device": device_id, "ticket": ticket}
                async with httpx.AsyncClient() as client:
                    res = await client.post(
                        URL_QUERY_GAME_TOKEN_QRCODE,
                        json=content,
        ...
        if is_incorrect_return(e):
            logger.exception(
                "查询米游社扫码登录(query_game_token_qrcode) - 服务器没有正确返回"
            )
            logger.debug(f"网络请求返回: {res.text}")
...
async def get_token_by_game_token(
    bbs_uid: str, game_token: str, retry: bool = True
    ...
    try:
        async for attempt in get_async_retry(retry):
            with attempt:
                content = {"account_id": int(bbs_uid), "game_token": game_token}
                async with httpx.AsyncClient() as client:
                    res = await client.post(
                        URL_GET_TOKEN_BY_GAME_TOKEN,
                ...
                if api_result.retcode == 0:
                    ...
                    return BaseApiStatus(success=True), BBSCookies(
                        stoken_v2=stoken_v2, mid=mid
                    )
                else:
                    logger.debug(f"网络请求返回: {res.text}")
                    return BaseApiStatus(), None
        ...
        if is_incorrect_return(e):
            logger.exception(
                "通过 GameToken 获取 SToken(get_token_by_game_token) - 服务器没有正确返回"
            )
            logger.debug(f"网络请求返回: {res.text}")
            return BaseApiStatus(incorrect_return=True), None
        else:
            logger.exception(
                "通过 GameToken 获取 SToken(get_token_by_game_token) - 请求失败"
            )
            return BaseApiStatus(network_error=True), None
...
async def get_cookie_token_by_game_token(
    ...
    try:
        ...
        if is_incorrect_return(e):
            logger.exception(
                "通过 GameToken 获取 CookieToken(get_cookie_token_by_game_token) - 服务器没有正确返回"
            )
            logger.debug(f"网络请求返回: {res.text}")
            return BaseApiStatus(incorrect_return=True), None
        else:
            logger.exception(
                "通过 GameToken 获取 CookieToken(get_cookie_token_by_game_token) - 请求失败"
            )
            return BaseApiStatus(network_error=True), None
