...
from utils import logger, generate_ds, get_async_retry, get_validate

URL_SIGN = "https://bbs-api.mihoyo.com/apihub/app/api/signIn"
URL_GET_POST = (
    "https://bbs-api.miyoushe.com/post/api/feeds/posts?fresh_action=1&gids={}&is_first_initialize=false"
    "&last_id="
)
URL_READ = "https://bbs-api.miyoushe.com/post/api/getPostFull?post_id={}"
URL_LIKE = "https://bbs-api.miyoushe.com/post/api/post/upvote"
URL_SHARE = (
    "https://bbs-api.miyoushe.com/apihub/api/getShareConf?entity_id={}&entity_type=1"
)
...
class BaseMission:
    ...
    async def sign(
        self, user: UserData, retry: bool = True
    ) -> Tuple[MissionStatus, Optional[int]]:
        """
        签到
        ...
        retrying.retry = retrying.retry and tenacity.retry_if_result(
            lambda x: x is None
        )
        try:
                ...
                with attempt:
                    ...
                    if api_result.login_expired:
                        ...
                        if plugin_config.preference.geetest_url or user.geetest_url:
                            create_status, mmt_data = await create_verification(
                                self.account
                            )
                            logger.debug(
                                "米游币任务 - 讨论区签到: "
                            ...
                            if create_status:
                                if geetest_result := await get_validate(
    ...
    ) -> Tuple[BaseApiStatus, Optional[List[str]]]:
        """
        获取文章ID列表，若失败返回 `None`
...
) -> Tuple[BaseApiStatus, Optional[List[MissionData]]]:
    """
    获取米游币任务信息
...
async def get_missions_state(
    account: UserAccount, retry: bool = True
) -> Tuple[BaseApiStatus, Optional[MissionState]]:
    """
    获取米游币任务完成情况
            ...
            with attempt:
                ...
                for mission in missions:
                    try:
                        current = list(
                            filter(
                                lambda state: state["mission_key"]
                                == mission.mission_key,
                                api_result.data["states"],
                            )
                        )[0]["happened_times"]
                        state_dict.setdefault(mission.mission_key, (mission, current))
                    except IndexError:
                        state_dict.setdefault(mission.mission_key, (mission, 0))
                return BaseApiStatus(success=True), MissionState(
                    state_dict=state_dict, current_myb=api_result.data["total_points"]
                )
    except tenacity.RetryError as e:
        if is_incorrect_return(e):
